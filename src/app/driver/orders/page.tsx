  ...
  const fetchOrders = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // On initial load, filter out orders that are already delivered
        const activeOrders = (data.orders || []).filter((order: Order) => order.status !== 'delivered');
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Don't automatically remove delivered orders - let driver hide them manually
          ...
          {orders.map((order) => (
            <div key={order.id} style={{ 
              ...
              <div>
                ...
                {/* Delete option for delivered orders */}
                {order.status === 'delivered' && (
                  <div style={{ 
                    ...
                    <span style={{ color: '#666', fontSize: '14px' }}>
                      Order delivered - you can hide it from your view
                    </span>
                    <button
